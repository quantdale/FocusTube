public enum MediaStreamFilter {
    ...
    public static func allowed(_ streams: [MediaStream], allowed: Set<Int> = allowedResolutions) -> [MediaStream] {
    ...
    public static func selectOnlineStream(_ media: ResolvedMedia, allowed: Set<Int> = allowedResolutions) -> MediaStream? {
        let candidates = allowed(media.combined, allowed: allowed).filter { $0.nativePlayable }
        return candidates.max { ($0.resolution ?? 0) < ($1.resolution ?? 0) }
    }

    /// Returns a copy with only allowed-resolution streams retained.
    public static func filter(_ media: ResolvedMedia, allowed: Set<Int> = allowedResolutions) -> ResolvedMedia {
        ResolvedMedia(
            videoID: media.videoID,
            extractedAt: media.extractedAt,
            combined: allowed(media.combined, allowed: allowed),
            videoOnly: allowed(media.videoOnly, allowed: allowed),
